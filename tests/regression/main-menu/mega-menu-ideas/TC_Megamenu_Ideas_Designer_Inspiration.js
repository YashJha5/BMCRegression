...
let tc_name = 'TC_Megamenu_Ideas_Designer_Inspiration';

describe('STORY: MEGAMENU IDEAS DESIGNER NSPIRATION', () => {
    ...
    it(test_data.tc_description, () => {
        ...
        executeStep('STEP ' + global.step_counter++ + ': MOUSE HOVER ON LINK: IDEAS', () => {
            CommonSteps.mouse_hover_on_main_menu_link(common_data.main_menu_index.ideas);        
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': DIV: MEGA MENU IS DISPLAYED', () => {
            assert.is_element_displayed(CommonPage.div_megamenu_advice_modal); 
        });
        executeStep('STEP ' + global.step_counter++ + ': CLICK LINK: IDEAS DESIGNER NSPIRATION', () => {
            CommonSteps.click_megamenu_ideas_model_menu_item_link(common_data.megamenu_ideas_index.designer_inspiration);
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': URL ENDPOINT', () => {
            assert.end_point(localized_data.navigation_endpoint.designer_inspiration);
        });
