...
let tc_name = 'TC_Megamenu_professionals_contractors';

describe('STORY: MEGAMENU PROFESSIONALS CONTRACTORS', () => {
    ...
    it(test_data.tc_description, () => {
        ...
        executeStep('STEP ' + global.step_counter++ + ': MOUSE HOVER ON LINK: PROFESSIONALS', () => {
            CommonSteps.mouse_hover_on_main_menu_link(common_data.main_menu_index.professionals);        
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': DIV: MEGA MENU IS DISPLAYED', () => {
            assert.is_element_displayed(CommonPage.div_megamenu_professional_modal); 
        });
        executeStep('STEP ' + global.step_counter++ + ': CLICK LINK: PROFESSIONALS CONTRACTORS', () => {
            CommonSteps.click_megamenu_professionals_model_menu_item_link(common_data.megamenu_professionals_index.contractors);
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': URL ENDPOINT', () => {
            assert.end_point(localized_data.navigation_endpoint.contractors);
        });
