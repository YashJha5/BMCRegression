...
let tc_name = 'TC_Megamenu_Color_Diy_Home_Projects';
...
describe('STORY: MEGAMENU COLOR OVERVIEW', () => {
    ...
    it(test_data.tc_description, () => {
        ...
        executeStep('STEP ' + global.step_counter++ + ': MOUSE HOVER ON LINK: COLOR', () => {
            CommonSteps.mouse_hover_on_main_menu_link(common_data.main_menu_index.color);        
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': DIV: MEGA MENU IS DISPLAYED', () => {
            assert.is_element_displayed(CommonPage.div_megamenu_color_modal); 
        });
        executeStep('STEP ' + global.step_counter++ + ': CLICK LINK: DIY HOME PROJECTS', () => {
            CommonSteps.click_megamenu_color_model_menu_item_link(common_data.megamenu_color_index.diy_home_projects);
        });
