...
let tc_name = 'TC_Megamenu_Color_Overview';
...
describe('STORY: MEGAMENU COLOR OVERVIEW', () => {
    ...
    it(test_data.tc_description, () => {
        ...
        executeStep('STEP ' + global.step_counter++ + ': CLICK LINK: COLOR OVERVIEW', () => {
            CommonSteps.click_megamenu_color_model_menu_item_link(common_data.megamenu_color_index.overview);
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': URL ENDPOINT', () => {
            assert.end_point(localized_data.navigation_endpoint.color);
        });
