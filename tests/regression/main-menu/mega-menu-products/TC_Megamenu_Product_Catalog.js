...
let tc_name = 'TC_Megamenu_Product_Catalog';

describe('STORY: MEGAMENU PRODUCT CATALOG', () => {
    ...
    it(test_data.tc_description, () => {
        ...
        executeStep('STEP ' + global.step_counter++ + ': CLICK LINK: PRODUCTS CATALOG', () => {
            CommonSteps.click_megamenu_products_model_menu_item_link(common_data.megamenu_products_index.product_catalog);
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': URL ENDPOINT', () => {
            assert.end_point(localized_data.navigation_endpoint.product_catalog);
        });
