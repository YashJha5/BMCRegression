...
let tc_name = 'TC_Megamenu_Products_Quality_Begins_With_Chemistry';

describe('STORY: MEGAMENU PRODUCTS QUALITY BEGINS WITH CHEMISTRY', () => {
    ...
    it(test_data.tc_description, () => {
        ...
        executeStep('STEP ' + global.step_counter++ + ': CLICK LINK: QUALITY BEGINS WITH CHEMISTRY', () => {
            CommonSteps.click_megamenu_products_model_menu_item_link(common_data.megamenu_products_index.quality_begins_with_chemistry);
        });
        executeStep('ASSERT ' + global.assert_counter++ + ': URL ENDPOINT', () => {
            assert.end_point(localized_data.navigation_endpoint.quality_begins_with_chemistry);
        });
